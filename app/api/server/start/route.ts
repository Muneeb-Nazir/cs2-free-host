  ...
  try {
    if (!COOLIFY_API || !COOLIFY_TOKEN || !CS2_SERVICE_UUID) {
      return NextResponse.json(
        { success: false, error: 'Coolify API not configured. Set COOLIFY_API_URL, COOLIFY_API_TOKEN, and CS2_SERVICE_UUID' },
        { status: 500 }
      );
    }
    ...
    const modeMap: Record<string, { mode: number; type: number }> = {
      casual: { mode: 0, type: 0 },
      competitive: { mode: 1, type: 0 },
      deathmatch: { mode: 2, type: 1 },
      wingman: { mode: 2, type: 0 },
      armsrace: { mode: 0, type: 1 },
      demolition: { mode: 1, type: 1 }
    };
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Server starting with configuration',
